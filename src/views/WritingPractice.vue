<template>
  <div class="flex h-full flex-col gap-4 px-4 py-4 lg:flex-row">
    <!-- 50音Menu -->
    <div class="w-full" :key="learningStore.writing_tab">
      <el-select
        v-model="learningStore.writing_tab"
        class="mb-4 w-full lg:max-w-lg"
        placeholder="選擇字符集"
    ...
    <div class="w-full content-center">
      <el-card>
        ...
        <div class="flex w-full items-center justify-between">
          ...
          >
            <span
              v-show="learningStore.writing_selectedSound.evo"
        ...
        <HandwritingCanvas
          ...
          :current-type="learningStore.writing_tab"
          :show-example="true"
...
const TAB_TYPES = {
  ...
  KATAKANA_HANDAKUON: "katakana_handakuon",
...
const tabGroups = [
  ...
  {
    label: t("advanced_learning"),
    options: [
      { name: TAB_TYPES.KATAKANA_DAKUON, label: "katakana_dakuon" },
      { name: TAB_TYPES.KATAKANA_HANDAKUON, label: "katakana_handakuon" },
      { name: TAB_TYPES.KATAKANA_YOON, label: "katakana_yoon" },
    ],
  },
];
...
const currentSounds = computed(
  () => fiftySounds[learningStore.writing_tab] ?? [],
);
...
const currentSounds_menu = computed(() => {
  ...
  let groupSize = 5;
  if (
    learningStore.writing_tab === TAB_TYPES.YOON ||
    learningStore.writing_tab === TAB_TYPES.KATAKANA_YOON
  ) {
    groupSize = 3;
  }
